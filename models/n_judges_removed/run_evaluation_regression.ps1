...
Get-ChildItem -Path "data/processed/n_judges_removed" -Filter "cof_selected_holdout_reg_noagg_*.csv" | 
ForEach-Object {
    $_.Name -match ".*?n(\d+)_(\d+)"
    $n = $Matches[1]
    $x = $Matches[2]
    $PATH_DATA_TEST = $_.FullName
    $PATH_DATA_BOOTSTRAP = "data/processed/n_judges_removed/cof_selected_all_reg_noagg_n$($n)_$($x).csv"
